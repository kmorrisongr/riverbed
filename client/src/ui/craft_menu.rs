...
impl Plugin for CraftMenuPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(SelectedRecipe(0))
            .insert_resource(HandCrafts(parse_recipes(
                &fs::read_to_string("assets/data/handcraft.recipe").unwrap(),
            )))
            .add_systems(OnEnter(GameUiState::CraftingMenu), open_craft_menu)
            .add_systems(
                OnExit(GameUiState::CraftingMenu),
                despawn_screen::<CraftingMenu>,
            )
            .add_systems(
                Update,
                (
                    ...
                    .chain()
                    .run_if(in_state(GameUiState::CraftingMenu)),
            );
    }
}
...
fn create_craft_menu(
    mut commands: Commands,
    inventory_recipes: InventoryRecipes,
    tex_map: Res<UiTextureMap>,
) {
    commands
        .spawn((
        ...
        .with_children(|parent| {
            ...
            for uncraftable_recipe in &inventory_recipes.uncraftable_entries {
                add_recipe_node(
                    parent,
                    uncraftable_recipe.get_example(0),
                    false,
                    &tex_map,
                    i,
                );
